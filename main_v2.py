...
if __name__ == "__main__":
    ...
    if RUN:
        ...
        for cluster in df.index:
            ...
            print(f'Iniciando agomerado: {cluster}')
            ...
            df.loc[cluster, 'er_bin_frac'] = calcula_er_fb(df, data.loc[cluster], 'bin_frac', cluster, q=0)
                                       
            df.loc[cluster, 'bin_frac_05'] = bin_frac(data.loc[cluster], q = 0.5)
            df.loc[cluster, 'bin_frac_05_corr'] = corr_fb_by_similarity(df, 'bin_frac_05', cluster,  k=5, av_lim=.5, dist_lim=1.5,)
            df.loc[cluster, 'er_bin_frac_05'] = calcula_er_fb(df, data.loc[cluster], 'bin_frac_05', cluster, q = 0.5)
            
            print('Calculo da fb concluido')
            print('Iniciando calculo das propriedades do aglomerado')
            ...
            print(f'Aglomerado {cluster} concluído')
            i+=1
        # Calcula tau
